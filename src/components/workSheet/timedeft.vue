                        ...
                        <el-dialog title="添加/修改" :visible.sync="detailVisible" width='400px' class="dialog-urge">
                            <div class="dialog-main">
                                <div class="revoke-reason">
                                    ...
                                    <mInput :list="systemList" :code.sync="detailInfo.devTypeCode" :name.sync="detailInfo.devTypeName" class="dialog-select"></mInput>
                                </div>
                                <div class="revoke-reason">
                                    <label class="dialog-label"><span>*</span>维修类型</label>
                                    <mInput :list="reptypeList" :code.sync="detailInfo.typeCode" :name.sync="detailInfo.typeName" class="dialog-select"></mInput>
                                </div>
                                <div class="revoke-reason">
    ...
    export default {
        ...
        methods: {
            ...
            searchPageInfo() {
                ...
                this.$api.get(`${this.$config.efoms_HOST}/workorderDeadline/selectDeadlineConfPage`, this.queryConditions, { token: this.token })
                    .then(res => {
                        setTimeout(load => {
            ...
            deleteItem() {
                this.$confirm('确认删除该条时间配置吗？', '操作提示', {
                    ...
                    this.$api.delete(`${this.$config.efoms_HOST}/workorderDeadline/deleteDeadlineConfById`, { deadlineConfId: this.detailInfo.deadlineConfId }, { token: this.token })
                        .then(res => {
                            if (res.appCode == 0) {
            ...
            saveItem() {
                ...
                if (this.detailInfo.deadlineConfId) {
                    ajxQA = this.$api.put(`${this.$config.efoms_HOST}/workorderDeadline/updateDeadlineConf`, this.detailInfo, { token: this.token, "Content-Type": "application/json;charset=UTF-8" });
                } else {
                    ajxQA = this.$api.post(`${this.$config.efoms_HOST}/workorderDeadline/insertDeadlineConf`, this.detailInfo, { token: this.token });
                }
                ajxQA.then(res => {
