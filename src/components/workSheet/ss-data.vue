<template>
    <div class="ej-main">
        <wkLayout ref="layout" :title="'工单查询'" :queryConditions="queryConditions" :totalPage="totalPage" :totalCount="totalCount" :searchMore="1" @searchTable="searchTableInfo" @searchPage="searchPageInfo">
            ...
            <template slot="table">
                <el-table :highlight-current-row="false" :data="tableData" border @current-change='currentSelect' class="content-table" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                    ...
                    <el-table-column prop="WORKORDERS_ID" label="工单流水号" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="CREATE_DATE" label="工单生成时间" show-overflow-tooltip min-width="120">
                    ...
                    <el-table-column prop="FINISH_TIME" label="完结时间" show-overflow-tooltip min-width="120">
                        <template slot-scope="scope">
                            {{scope.row.FINISH_TIME|dateTime}}
                        </template>
                    </el-table-column>
                </el-table>
            </template>
    ...
    export default {
        ...
        methods: {
            ...
            searchPageInfo() {
                ...
                this.$api.get(`${this.$config.efoms_HOST}/workordersRecord/getWorkordersCount`, this.queryConditions, { token: this.token })
                    .then(res => {
                        setTimeout(load => {
