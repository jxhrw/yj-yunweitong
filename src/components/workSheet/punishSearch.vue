<template>
    <div class="ej-main">
        <WkLayout ref="layout" :title="'处罚工单'" :queryConditions="queryConditions" :totalPage="totalPage" :totalCount="totalCount" :searchMore="1" @searchTable="searchTableInfo" @searchPage="searchPageInfo">
            <template slot="condFirst">
                ...
                <el-col :span="7">
                    <label style="white-space: nowrap;">申报编号</label>
                ...
                <el-col :span="7">
                    <label>申报部门</label>
                    <mInput class="spc-height" :list="departList" :code.sync="departCode" showAttr="deptName" getAttr="deptId" @keyup.enter.native="searchTableInfo"></mInput>
                </el-col>
            </template>
            ...
            <template slot="table">
                <div style="text-align: right;font-size: 14px;margin-bottom: 10px;color: #909399;">总处罚金额：{{moneyNum}}元</div>
                <el-table :highlight-current-row="false" :data="tableData" border @current-change='currentSelect' class="content-table" v-loading="isTableLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255, 255, 255, 0.7)">
                    <el-table-column prop="REP_DEPT_NAME" label="申报编号" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="GDZC" label="申报单位" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="YWC" label="所属系统" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="WWC" label="申报来源" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="XFL" label="所属辖区" show-overflow-tooltip>
                        <template slot-scope="scope">
                            {{(scope.row.XFL&&scope.row.XFL>0)?`${scope.row.XFL}%`:scope.row.XFL}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="JSXFL" label="情况描述" show-overflow-tooltip>
                        <template slot-scope="scope">
                            {{(scope.row.JSXFL&&scope.row.JSXFL>0)?`${scope.row.JSXFL}%`:scope.row.JSXFL}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="CSGD" label="地址" show-overflow-tooltip></el-table-column>
                    <el-table-column prop="GBGD" label="申报时间" show-overflow-tooltip min-width="120"> </el-table-column>
                    <el-table-column prop="GBGD" label="状态" show-overflow-tooltip> </el-table-column>
                    <el-table-column prop="GBGD" label="处罚金额" show-overflow-tooltip> </el-table-column>
                </el-table>
            </template>
        </WkLayout>
    </div>
