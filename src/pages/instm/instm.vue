...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$api.post(`${this.$config.uums_HOST}/user/heartbeat?token=${token}&systemKey=${this.$config.systemKeySystem}`, {}, { token: token });
            setInterval(() => {
                this.$api.post(`${this.$config.uums_HOST}/user/heartbeat?token=${token}&systemKey=${this.$config.systemKeySystem}`, {}, { token: token });
            }, 5 * 60 * 1000);
        },
        methods: {
            ...
            getUserInfo() {
                ...
                let token = Common.getQueryString("token");
                this.$api.getMethod(host, method, { token: token, systemKey: this.$config.systemKeySystem }, token).then(res => {
                    if (res.appCode == 0) {
                            ...
                            this.rightListsEx = [{
                                children: [{
                                    ...
                                    children: [{
                                        checked: true,
                                        rightName: "维修申报",
                                        rightUrl: 'sheetxt?type=0'
                                    }, {
                                        checked: true,
                                        rightName: "维修处置",
                                        rightUrl: 'sheetxt?type=11'
                                    }, {
                                        checked: true,
                                        rightName: "工单确认",
                                        rightUrl: 'sheetxt?type=12'
                                    }, {
                                        checked: true,
                                        rightName: "工单查询",
                                        rightUrl: 'sheetxt?type=7'
                                    }]
                                }]
                            }]
                        }
            ...
            tipsPush(code) {
                ...
                ServerPush.receive(code, 'message', (res) => {
                    ...
                    if (isPower && res.workorderType == 'system') {
                        this.domIndex += 1;
                        ...
                        .then(res => {
                            if (res.appCode == 0) {
                                ...
                                if (tableData.length > 0) {
                                    sessionStorage.setItem('transferxtInfo', JSON.stringify(tableData[0]));
                                    this.$router.push({
                                        path: '/detsheetxt',
